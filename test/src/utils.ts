...
export const colorMap: Record<LightColor, string> = {
  red: '#ff4444',
  green: '#44ff44',
  blue: '#4488ff',
  yellow: '#ffdd44',
...
export function calculateLightPath(
  ...
  function traceLightRay(startX: number, startY: number, angle: number, color: LightColor, bounces: number) {
    ...
    for (const element of elements) {
      ...
      } else if (element.type === 'twoWayMirror') {
        hit = rayMirrorIntersection(startX, startY, dirX, dirY, element);
      } else if (element.type === 'target' || element.type === 'splitter' || element.type === 'colorFilter' || element.type === 'obstacle' || element.type === 'lightDetector' || element.type === 'beamSplitter') {
    ...
    if (closestHit) {
      ...
      if (hitElement) {
        ...
        } else if (hitElement.type === 'beamSplitter') {
          const reflectedAngle = reflectAngle(angle, hitElement.angle);
          traceLightRay(closestHit.x, closestHit.y, reflectedAngle, color, bounces + 1);
          traceLightRay(closestHit.x, closestHit.y, angle, color, bounces + 1);
        ...
        } else if (hitElement.type === 'twoWayMirror') {
          if (Math.random() > 0.5) {
            const reflectedAngle = reflectAngle(angle, hitElement.angle);
            traceLightRay(closestHit.x, closestHit.y, reflectedAngle, color, bounces + 1);
          } else {
        ...
        } else if (hitElement.type === 'lens') {
          const focalLength = hitElement.focalLength || 50;
          const newAngle = refractThroughLens(angle, closestHit.x, closestHit.y, hitElement.x, hitElement.y, focalLength);
          traceLightRay(closestHit.x, closestHit.y, newAngle, color, bounces + 1);
        } else if (hitElement.type === 'lightDetector') {
          hitTargets.add(hitElement.id);
        }
      }
    }
...
function refractThroughLens(angle: number, hitX: number, hitY: number, lensX: number, lensY: number, focalLength: number): number {
  const dx = hitX - lensX;
  const dy = hitY - lensY;
  const distFromCenter = Math.sqrt(dx * dx + dy * dy);
  
  if (distFromCenter < 5) return angle;
  
  const angleToCenter = Math.atan2(dy, dx);
  const deflectionAngle = (distFromCenter / 30) * (focalLength > 0 ? 1 : -1);
  
  return toDegrees(angleToCenter + deflectionAngle);
}